...
const Dashboard = (props) => {
  ...
  const postMasterFormId = "SV_51kYr8YiMnAJkIS"; // regoption = 1
  const topNBottomFormId = "SV_3kocaAWBWNheeqy"; // regoption = 2 
  const BottomFormId = "SV_0le77iNtacKfMLs"; // regoption = 4
  ...
  const [backToRegSurvey, setBackToRegSurvey] = useState(false);
  ...
  // Get Registration survey redirect query string
  useEffect(() => {
    console.log('ssssssssssss:',query.get("o"))
    if (query.get("o")) {
      console.log('kllllllllll')
      setBackToRegSurvey(true);
    }else{
      setBackToRegSurvey(false);
    }
  }, [query.get("o")]);
  ...
  return (
    <div className="main_content_dashboard">
      ...
      {(!deputySurveyLink && !regOption && qualtricsUserId && !userSurveyId) || (backToRegSurvey) ? (
        <div>
          <iframe
            id="qualtricsId"
            className="test"
            src={`https://peacefully.qualtrics.com/jfe/form/SV_bOxiUrOw21xaDTo?fname=${firstName}&lname=${lastName}&email=${email}&contact_email=${email}&contact_mobile=${mobile}&user_id=${userId}&CONTACT_ID=${qualtricsUserId}`}
          ></iframe>
        </div>
      ) : null}
        ...
        <div>
          <iframe
            ...
            src={`https://peacefully.qualtrics.com/jfe/form/${
              regOption == 1 ? postMasterFormId : regOption == 2 ? topNBottomFormId : BottomFormId
